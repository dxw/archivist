describe Archivist::Client do
  ...
  let(:channel) { Slack::Messages::Message.new(id: "test-id", name: "test") }

  let(:blocks) { [{ type: "section", text: { type: "plain_text", text: "Some words" } }] }
  ...
  describe ".list_public_channels" do
    it "calls conversations_list on the Slack client" do
      expect(subject.slack_client).to receive(:conversations_list).with(
  ...
  describe ".join" do
    it "calls conversations_join on the Slack client" do
      expect(subject.slack_client).to receive(:conversations_join).with(channel: channel.id)
  ...
  describe ".leave" do
    it "calls conversations_leave on the Slack client" do
      expect(subject.slack_client).to receive(:conversations_leave).with(channel: channel.id)
  ...
  describe ".archive" do
    it "calls conversations_archive on the Slack client" do
      expect(subject.slack_client).to receive(:conversations_archive).with(channel: channel.id)
  ...
  describe ".post_to" do
    it "calls chat_postMessage on the Slack client" do
      expect(subject.slack_client).to receive(:chat_postMessage).with(channel: channel.id, blocks: blocks)
  ...
  describe ".last_messages_in" do
    it "calls conversations_history on the Slack client" do
      expect(subject.slack_client).to receive(:conversations_history).with(
        channel: channel.id,
        limit: 123,
      ...
      subject.last_messages_in(channel, limit: 123, min_days_ago: 12, max_days_ago: 23)
    end
  end
end
