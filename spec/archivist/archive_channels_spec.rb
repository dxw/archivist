describe Archivist::ArchiveChannels do
  ...
  let(:report_channel) { Slack::Messages::Message.new(id: "report-test-id", name: "report-test") }
  let(:active_channel) { Slack::Messages::Message.new(id: "active-test-id", name: "active-test") }
  let(:stale_channel) { Slack::Messages::Message.new(id: "stale-test-id", name: "stale-test") }
  let(:warned_active_channel) { Slack::Messages::Message.new(id: "warned-active-test-id", name: "warned-active-test") }
  let(:warned_stale_channel) { Slack::Messages::Message.new(id: "warned-stale-test-id", name: "warned-stale-test") }
  let(:member_channel) { Slack::Messages::Message.new(id: "member-test-id", name: "member-test", is_member: true) }
  let(:general_channel) { Slack::Messages::Message.new(id: "general-test-id", name: "general-test", is_general: true) }
  let(:shared_channel) { Slack::Messages::Message.new(id: "shared-test-id", name: "shared-test", is_shared: true) }
  ...
  let(:skip_channel) { Slack::Messages::Message.new(id: "skip-test-id", name: "skip-test") }

  let(:conversations_list_responses) do
    [
      Slack::Messages::Message.new(
        channels: [
          report_channel,
      ...
      Slack::Messages::Message.new(channels: [shared_channel, pending_shared_channel, skip_channel])
    ]
  end
  let(:active_conversations_history_responses) do
    [
      ...
      Slack::Messages::Message.new(
        messages: [Slack::Messages::Message.new(bot_id: "testbotid", ts: Time.now.to_f.to_s)]
      )
    ]
  end
    ...
    [
      Slack::Messages::Message.new(
        messages: [
          Slack::Messages::Message.new(subtype: "channel_join", ts: Time.now.to_f.to_s),
          Slack::Messages::Message.new(subtype: "channel_leave", ts: Time.now.to_f.to_s),
          Slack::Messages::Message.new(bot_id: Archivist::Channel::SLACKBOT_BOT_ID, ts: Time.now.to_f.to_s),
          Slack::Messages::Message.new(hidden: true, ts: Time.now.to_f.to_s)
        ]
      ),
      Slack::Messages::Message.new(
        messages: [Slack::Messages::Message.new(bot_id: Archivist::Channel::SLACKBOT_BOT_ID, ts: Time.now.to_f.to_s)]
      )
    ]
  end
  ...
  before do
    ...
    allow(Archivist::Config).to receive(:report_channel_id) { report_channel.id }
  ...
  describe "#run with default rules" do
    ...
    it "joins the report channel" do
      expect(slack_client).to receive(:conversations_join).with(channel: report_channel.id)

      subject.run
    end

    it "joins channels it's not already a member of" do
      expect(slack_client).to receive(:conversations_join).with(channel: active_channel.id)
      expect(slack_client).to receive(:conversations_join).with(channel: stale_channel.id)

      subject.run
    end

    it "doesn't join channels it's already a member of" do
      expect(slack_client).not_to receive(:conversations_join).with(channel: member_channel.id)

      subject.run
    end

    it "doesn't join the general channel" do
      expect(slack_client).not_to receive(:conversations_join).with(channel: general_channel.id)

      subject.run
    end

    it "doesn't join shared or pending shared channels" do
      expect(slack_client).not_to receive(:conversations_join).with(channel: shared_channel.id)
      expect(slack_client).not_to receive(:conversations_join).with(channel: pending_shared_channel.id)

      subject.run
    end

    it "doesn't join channels marked as skip by a matching rule" do
      expect(slack_client).not_to receive(:conversations_join).with(channel: skip_channel.id)
    ...
    it "uses activity no older than 30 days when deciding whether a channel is stale by default" do
      Timecop.freeze do
        expect(slack_client).to receive(:conversations_history).with(
    ...
    it "warns stale channels" do
      expect(slack_client).to receive(:chat_postMessage).with(channel: stale_channel.id, blocks: warning_message_blocks)

      subject.run
    end

    it "doesn't warn active channels" do
      expect(slack_client).not_to receive(:chat_postMessage).with(
        channel: active_channel.id,
        blocks: warning_message_blocks
      )
      expect(slack_client).not_to receive(:chat_postMessage).with(
        channel: warned_active_channel.id,
        blocks: warning_message_blocks
      )
    ...
    it "doesn't warn warned channels" do
      ...
      expect(slack_client).not_to receive(:chat_postMessage).with(
        channel: warned_stale_channel.id,
        blocks: warning_message_blocks
      )
    ...
    it "archives warned stale channels" do
      expect(slack_client).to receive(:conversations_archive).with(channel: warned_stale_channel.id)

      subject.run
    end

    it "doesn't archive active channels" do
      expect(slack_client).not_to receive(:conversations_archive).with(channel: active_channel.id)

      subject.run
    end

    it "doesn't archive stale but unwarned channels" do
      expect(slack_client).not_to receive(:conversations_archive).with(channel: stale_channel.id)

      subject.run
    end

    it "doesn't archive warned active channels" do
      expect(slack_client).not_to receive(:conversations_archive).with(channel: warned_active_channel.id)

      subject.run
    end

    it "doesn't archive the general channel" do
      expect(slack_client).not_to receive(:conversations_archive).with(channel: general_channel.id)

      subject.run
    end

    it "doesn't archive shared or pending shared channels" do
      expect(slack_client).not_to receive(:conversations_archive).with(channel: shared_channel.id)
      expect(slack_client).not_to receive(:conversations_archive).with(channel: pending_shared_channel.id)
    ...
    it "sends a report to the report channel" do
      expect(slack_client).to receive(:chat_postMessage).with(
        ...
        blocks: [
          ...
          { type: "section", text: { type: "mrkdwn", text: ":file_folder: #warned-stale-test" } }
        ]
      )
      expect(slack_client).to receive(:chat_postMessage).with(
        channel: report_channel.id,
        blocks: [
          ...
          { type: "section", text: { type: "mrkdwn", text: ":open_file_folder: #stale-test" } }
        ]
      )
  ...
  describe "#run without default rules" do
    ...
    it "doesn't join channels not covered by the rules" do
      expect(slack_client).not_to receive(:conversations_join).with(channel: active_channel.id)
    ...
    it "leaves any channels not covered by the rules" do
      expect(slack_client).to receive(:conversations_leave).with(channel: member_channel.id)
